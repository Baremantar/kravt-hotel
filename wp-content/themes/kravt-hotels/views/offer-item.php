...
<main style="background-color: white;">
    ...
    <div class="container kravt_offerItem_list-container custom">
        <div>
            <ul>
                <?php foreach ($dom->find('li') as $element) {echo $element->outertext();} ?>
            </ul>

            <?php 
                $booking_offer_link = null;
                foreach ($dom->find('a') as $i => $element) {
                    $href = $element->href;
                    if ($href) { 
                        $booking_offer_link = $href;
                    }
                }

                if ($booking_offer_link):
            ?>
            <a class="kravt_button-border button-roompage booking-offer-link" href="window.open(<?= $booking_offer_link ?>, '_blank')">Забронировать</a>
            <?php
                else:
            ?>
            <button type="button" class="kravt_button-border <?php if(get_site()->blog_id != 1) {echo '_booking-link';} else {echo'button-roompage';}?>" <?php if(get_site()->blog_id == 1) {echo 'onclick="document.querySelector(`tl-search-form`).querySelector(`.sfl-submit-button.sfl-submit-button-input.standard-view-button`).click()"';} ?>>Забронировать</button>
            <?php
                endif;
            ?>
        </div>
    ...
    <div class="container">
        ...
        <div class="kravt_offerItem_section-container-3">
            <div>
                <?php foreach ($dom->find('span') as $i => $element) {echo $element->outertext();} ?>
            </div>
        </div>
        <!-- <button type="button" class="kravt_button-border _offerItem-button-mobile _booking-link">Узнать</button> -->
    </div>
