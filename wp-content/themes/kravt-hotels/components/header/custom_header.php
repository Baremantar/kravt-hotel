...
if (!is_404()) {
    if (get_post()->post_name == 'excursion') {
        $value =  'kravt_excursion_wrapper';
    } else if (get_the_category() != NULL) {
    ...
    } else if($actual_link[1] == '/services/' ||
                $actual_link[1] == '/offers/' ||
                $actual_link[1] == '/events/'
                ){
        $value = 'body-white';
    } else {
        $value = '_body-main';
...
switch (get_site()->blog_id) {
    ...
    case 5:
        ...
        break;
    default:
        $logoIcon = file_get_contents(get_template_directory_uri() . '/build/img/logos/KH.svg');
        break;
};
...
if (explode('.', $_SERVER['HTTP_HOST'])[0] == 'sadovaya') {
...
?>
<body class="<?= $value ?>">
    <header class="kravt_header <?php if(get_site()->blog_id == 5){echo 'sadovaya';}?>" id="kravt_header" <?php if(is_404()){echo 'data-negative="1"';} ?>>
        <div class="navbar_scrolled_mask" id="scrolled_mask"></div>
        <?php if (explode('?', $_SERVER['REQUEST_URI'])[0] != '/') { if(!$ifMobile()){?><div class="kravt_wrapper"><?php }} ?>
            <nav class="kravt_content kravt_navbar container" id="header">
                <div id="navbar_darked" class="navbar_darked"></div>
                <div class="kravt-logo" onclick="window.location.href = window.location.origin" >
                        <?= $logoIcon ?>
                </div>
                <div class="kravt_navigation" id="navigation_links">
                    ...
                    <div class="kravt_dropdown" id="header_picker">
                        ...
                        <div class="kravt_dropdown_content kravt_dropdown-hotels rounded">
                            <a href="/rooms/" class="kravt_dropdown_link">
                                <p>Посмотреть все</p>
                            </a>
                ...
                <a href="/lk" id="lk_button-inner" class="kravt_hero_phone-button">
                    Личный кабинет
                </a>
                ...
                <div class="kravt_burger_header">
                    ...
                    <div class="kravt_social_wrapper">
                        <img src="<?= get_template_directory_uri() . '/build/img/socials/vk.svg' ?>" alt="VK"
                            title="VKontakte" onclick="window.open('<?= $link[0]?>','_blank')"/>
                        <img src="<?= get_template_directory_uri() . '/build/img/socials/tg.svg' ?>" alt="TG"
                            title="Telegram" onclick="window.open('<?= $link[1]?>','_blank')"/>
                    </div>
            ...
            <?php if (explode('?', $_SERVER['REQUEST_URI'])[0] !== '/') { if(!$ifMobile() || explode('/', $_SERVER['REQUEST_URI'])[1] == 'rooms'){?>
